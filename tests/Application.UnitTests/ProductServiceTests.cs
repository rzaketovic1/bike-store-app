...
public class ProductServiceTests
{
    ...
    public async Task GetProductByIdAsync_ShouldThrowNotFoundException_WhenProductNotFound()
    {
        ...
        // Act
        var act = async () => await _service.GetProductByIdAsync(999);
    ...
    public async Task CreateProductWithImageAsync_ShouldUploadImageAndCreateProduct()
    {
        ...
        // Assert
        _fileUploadMock.Verify(f => f.IsValidImage(fileMock.Object), Times.Once);
        _fileUploadMock.Verify(f => f.UploadImageAsync(fileMock.Object, "products"), Times.Once);
        _repoMock.Verify(r => r.AddProduct(It.Is<Product>(p => p.PictureUrl == "unique-filename.jpg")), Times.Once);
        _repoMock.Verify(r => r.SaveChangesAsync(), Times.Once);
        result.Name.Should().Be("Test Bike");
        result.PictureUrl.Should().Be("/images/products/unique-filename.jpg");
    }

    [Fact]
    public async Task CreateProductWithImageAsync_ShouldThrowBadRequestException_WhenImageIsInvalid()
    {
        // Arrange
        var fileMock = CreateMockFormFile("bike.jpg", 1024);
        var dto = CreateProductWithImageDto("Test Bike", fileMock.Object);

        _fileUploadMock.Setup(f => f.IsValidImage(fileMock.Object)).Returns(false);
        ...
        _fileUploadMock.Verify(f => f.UploadImageAsync(It.IsAny<IFormFile>(), It.IsAny<string>()), Times.Never);
        _repoMock.Verify(r => r.AddProduct(It.IsAny<Product>()), Times.Never);
        _repoMock.Verify(r => r.SaveChangesAsync(), Times.Never);
    }

    [Fact]
    public async Task CreateProductWithImageAsync_ShouldThrowBadRequestException_WhenImageIsNull()
    {
        // Arrange
    ...
    public async Task UpdateProduct_ShouldThrowNotFoundException_WhenProductNotFound()
    {
        ...
        // Act
        var act = async () => await _service.UpdateProduct(999, updateDto);
    ...
    public async Task UpdateProductWithImageAsync_ShouldReplaceImage_WhenNewImageProvided()
    {
        ...
        _fileUploadMock.Verify(f => f.UploadImageAsync(fileMock.Object, "products"), Times.Once);
        _repoMock.Verify(r => r.SaveChangesAsync(), Times.Once);
        result.Name.Should().Be("Updated Bike");
        result.Price.Should().Be(899.99m);
        result.Brand.Should().Be("Specialized");
        result.Type.Should().Be("Road");
        result.QuantityInStock.Should().Be(7);
        result.PictureUrl.Should().Be("/images/products/new-unique-filename.jpg");
    }

    [Fact]
    public async Task UpdateProductWithImageAsync_ShouldKeepExistingImage_WhenNoNewImageProvided()
    {
        // Arrange
        var existingProduct = CreateTestProduct(1, "Test Bike", 599.99m);
        existingProduct.PictureUrl = "old-image.jpg";
        ...
        // Assert
        _fileUploadMock.Verify(f => f.UploadImageAsync(It.IsAny<IFormFile>(), It.IsAny<string>()), Times.Never);
        _fileUploadMock.Verify(f => f.DeleteImage(It.IsAny<string>(), It.IsAny<string>()), Times.Never);
        existingProduct.PictureUrl.Should().Be("old-image.jpg");
        result.Name.Should().Be("Updated Bike");
        result.Price.Should().Be(799.99m);
        result.Brand.Should().Be("Giant");
        result.Type.Should().Be("Hybrid");
        result.QuantityInStock.Should().Be(9);
        result.PictureUrl.Should().Be("/images/products/old-image.jpg");
        _repoMock.Verify(r => r.SaveChangesAsync(), Times.Once);
    }

    [Fact]
    public async Task UpdateProductWithImageAsync_ShouldThrowBadRequestException_WhenNewImageIsInvalid()
    {
        // Arrange
        var existingProduct = CreateTestProduct(1, "Test Bike", 599.99m);
        existingProduct.PictureUrl = "old-image.jpg";
        ...
        // Act
        var act = async () => await _service.UpdateProductWithImageAsync(1, dto);
    ...
    [Fact]
    public async Task UpdateProductWithImageAsync_ShouldThrowNotFoundException_WhenProductNotFound()
    {
        // Arrange
        var dto = new ProductWithImageDto
        {
        ...
        // Act
        var act = async () => await _service.UpdateProductWithImageAsync(999, dto);
    ...
    public async Task DeleteProduct_ShouldThrowNotFoundException_WhenProductNotFound()
    {
        ...
        // Act
        var act = async () => await _service.DeleteProduct(999);
